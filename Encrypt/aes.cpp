...
namespace Encrypt
{
	namespace AES
	{
		...
		*/
		...
		{
			auto len = strlen(src);
			int count = len / (4 * Nb) + (len % (4 * Nb) == 0 ? 0 : 1);

			uint8_t* out = new uint8_t[(count * 4 * Nb + 1) * sizeof(uint8_t)];
			memset(out, 0x00, (count * 4 * Nb + 1) * sizeof(uint8_t));
			static uint8_t* w = nullptr;
			static bool flag = true;
			if (flag)
			{
				flag = false;
				w = aes_init(strlen(key) * sizeof(char));
			...
			{
				aes_cipher((uint8_t*)src + i * 4 * Nb, (uint8_t*)out + i * 4 * Nb, w);
			}
			
			auto sData = Data::encodeByType(out, count * 4 * Nb, type);
			delete[] out;
			//free(w);
		...
		{
			int len = 0;
			auto sEncode = Data::decodeByType(src, len, type);
			int count = len / (4 * Nb) + (len % (4 * Nb) == 0 ? 0 : 1);
			
			uint8_t* w = aes_init(strlen(key) * sizeof(char));
			aes_key_expansion((uint8_t*)key, w);
			...
			for (int i = 0; i < count; i++)
			{
				aes_inv_cipher((uint8_t*)sEncode + i * 4 * Nb, (uint8_t*)out + i * 4 * Nb, w);
			}
...
#else
			std::string sData;
			sData.resize((count * 4 * Nb + 1) * sizeof(uint8_t) + 1);

			// decode
			for (int i = 0; i < count; i++)
			{
				aes_inv_cipher((uint8_t*)sEncode.data() + i * 4 * Nb, (uint8_t*)sData.data() + i * 4 * Nb, w);
			}
